...
type ButtonProps = {
  ...
  variant?: 'fill' | 'outline_pink' | 'outline_blue' | 'text' | 'side_bar' | 'shelf' | 'play';  //Button variants
  className?: string;
  href?: string; 
  Icon?: ElementType
...
export default function Button({
    ...
    const variantStyles = {
      ...
      play:'absolute w-14 h-14 hover:scale-110 shadow-black shadow-sm bg-green-500 hover:bg-green-400 rounded-full flex items-center justify-center cursor-pointer opacity-0 group-hover:opacity-100 transition-opacity duration-300 top-25 right-4',
    };
    ...
    const iconStyles = clsx(
      ...
      { [ "h-12 w-12 lg:h-6 lg:w-6"] : raw && variant==="shelf" }, 
      { [ "w-0 h-0 border-l-[20px] border-l-black border-t-[12px] border-t-transparent border-b-[12px] border-b-transparent"] : raw && variant==="play" },
    );
    
    //Styles applied to button children
    const childrenStyles = clsx(
      { [ "absolute mt-48 lg:mt-24 text-center text-xl lg:text-lg"] : variant==="shelf"}
    );
